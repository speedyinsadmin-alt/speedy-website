...
async function sendRefundEmail(o) {
  ...
  try {
    ...
    await t.sendMail({
      ...
      html,
      /* the confirmation slip, when the caller has one — the same PDF filed to HawkSoft */
      ...(Array.isArray(o.attachments) && o.attachments.length ? { attachments: o.attachments } : {}),
...
/* ================= THE REFUND CONFIRMATION SLIP (Sep 15) =================
   The refund's counterpart of the payment receipt. A charge already gets a branded
   receipt filed three ways (HawkSoft attachment, vault row, event); a refund got a log
   note and nothing the client could hold. Saif: a client with no email needs something
   to be handed, and the audit needs the same document in HawkSoft next to the receipt.

   Built from the refund row's OWN FACTS — amount, which payment, card ending, Clover
   reference, reason, partial / full / cash / void — never from anything typed, so it
   cannot be made to say what the ledger does not. Same slip size and brand as the
   receipt (306 x 590) so the two sit together. English is filed the moment the refund
   succeeds; Spanish is rendered from the same facts the first time someone asks for it,
   and filed then. A slip made more than an hour after the refund says so in its footer
   ("generated afterwards"), so a back-filled one never passes for a same-day one. */
const SLIP_T = {
  en: {
    title: 'REFUND CONFIRMATION', voidTitle: 'CHARGE CANCELLED',
    headCard: { full: 'Full refund to your {card} ending {last4}', partial: 'Partial refund to your {card} ending {last4}', void: 'Nothing will post to your {card} ending {last4}' },
    headCash: { full: 'Returned to you in cash, in full', partial: 'Returned to you in cash, in part' },
    date: 'Date / Time', client: 'Client', clientNo: 'Client #', reason: 'Reason',
    refundRef: 'Refund reference', ofPayment: 'Of the payment', paidOn: 'Paid on', payRef: 'Payment reference',
    stands: 'Still stands', returned: 'Returned to you', by: 'Issued by', record: 'ORIGINAL PAYMENT',
    partLine: 'This is a partial refund: {ret} of your {of} payment has been returned; {stands} stands.',
    fullLine: 'Your {of} payment has been returned in full.',
    voidLine: 'The {of} charge was cancelled before it settled. It will not appear on your statement; if a pending hold shows, your bank releases it within a few days.',
    bankCard: 'Refunds appear on the card used for the original payment, usually within 2 to 5 business days, as a credit from Speedy Insurance.',
    bankCash: 'The cash was handed back by your Speedy office; keep this slip as your record.',
    quote: 'Questions? Contact your Speedy office and quote the {ref} above.',
    refRefund: 'refund reference', refPayment: 'payment reference',
    made: 'generated by the Speedy platform', late: 'generated afterwards from the refund record',
    thanks: 'Thank you for choosing Speedy Insurance!',
    reasons: { charged_twice: 'Charged twice', wrong_amount: 'Wrong amount taken', policy_cancelled: 'Policy cancelled', never_bound: 'Policy never bound', other: 'Other' },
  },
  es: {
    title: 'CONFIRMACIÓN DE REEMBOLSO', voidTitle: 'CARGO CANCELADO',
    headCard: { full: 'Reembolso completo a su {card} terminada en {last4}', partial: 'Reembolso parcial a su {card} terminada en {last4}', void: 'No se cobrará nada a su {card} terminada en {last4}' },
    headCash: { full: 'Devuelto a usted en efectivo, por completo', partial: 'Devuelto a usted en efectivo, en parte' },
    date: 'Fecha / Hora', client: 'Cliente', clientNo: 'Cliente #', reason: 'Motivo',
    refundRef: 'Referencia del reembolso', ofPayment: 'Del pago de', paidOn: 'Pagado el', payRef: 'Referencia del pago',
    stands: 'Queda vigente', returned: 'Devuelto a usted', by: 'Emitido por', record: 'PAGO ORIGINAL',
    partLine: 'Este es un reembolso parcial: se le devolvieron {ret} de su pago de {of}; {stands} queda vigente.',
    fullLine: 'Su pago de {of} fue devuelto por completo.',
    voidLine: 'El cargo de {of} se canceló antes de procesarse. No aparecerá en su estado de cuenta; si ve una retención pendiente, su banco la libera en pocos días.',
    bankCard: 'Los reembolsos aparecen en la tarjeta del pago original, normalmente en 2 a 5 días hábiles, como un crédito de Speedy Insurance.',
    bankCash: 'El efectivo se lo entregó su oficina Speedy; conserve este comprobante como su registro.',
    quote: '¿Preguntas? Comuníquese con su oficina Speedy y mencione la referencia {ref}.',
    refRefund: 'del reembolso', refPayment: 'del pago',
    made: 'generado por la plataforma Speedy', late: 'generado después, a partir del registro del reembolso',
    thanks: '¡Gracias por elegir Speedy Insurance!',
    reasons: { charged_twice: 'Cobro duplicado', wrong_amount: 'Se cobró un monto incorrecto', policy_cancelled: 'Póliza cancelada', never_bound: 'Póliza nunca emitida', other: 'Otro' },
  },
};
const slipMoney = n => '$' + Math.abs(Number(n || 0)).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
const slipFill = (t, o) => String(t).replace(/\{(\w+)\}/g, (_, k) => o[k] != null ? o[k] : '');
/* A Pacific stamp the client can read, "2026-09-15 16:02". */
function slipStamp(ts) {
  try {
    const p = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Los_Angeles', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).formatToParts(new Date(ts));
    const g = k => (p.find(x => x.type === k) || {}).value || '';
    return `${g('year')}-${g('month')}-${g('day')} ${g('hour')}:${g('minute')}`;
  } catch { return String(ts || '').slice(0, 16).replace('T', ' '); }
}
/* pdf-lib's standard fonts are WinAnsi: accented Spanish is fine, a real minus sign or
   a curly quote is not. Everything drawn goes through this. */
const slipAscii = t => String(t).replace(/[\u2212]/g, '-').replace(/[\u2018\u2019]/g, "'").replace(/[\u201c\u201d]/g, '"').replace(/[\u2014]/g, '-').replace(/[^\x00-\xff]/g, '?');

async function buildRefundSlipPdf(o) {
  const { PDFDocument, StandardFonts, rgb } = await import('pdf-lib');
  const t = SLIP_T[o.lang] || SLIP_T.en;
  ...
  const RED = rgb(0.83, 0.17, 0.17), NAVY = rgb(0.10, 0.14, 0.34), GRAY = rgb(0.4, 0.4, 0.4),
        ...
        LINE = rgb(0.8, 0.8, 0.8), AMBER = rgb(0.64, 0.36, 0);
  const W = 306, H = 590; const pg = doc.addPage([W, H]);
  const isVoid = o.kind === 'void', isPart = o.kind === 'partial', isCard = o.method === 'card';
  const draw = (s, x, y, f, sz, c) => pg.drawText(slipAscii(s), { x, y, font: f, size: sz, color: c });
  const wid = (s, f, sz) => f.widthOfTextAtSize(slipAscii(s), sz);
  const ctr = (s, y, f, sz, c) => draw(s, (W - wid(s, f, sz)) / 2, y, f, sz, c);
  const dash = y => pg.drawLine({ start: { x: 25, y }, end: { x: W - 25, y }, thickness: 1, color: LINE, dashArray: [2, 2] });
  const solid = y => pg.drawLine({ start: { x: 25, y }, end: { x: W - 25, y }, thickness: 1, color: LINE });
  let y = H - 32;
  ctr('SPEEDY', y, boldObl, 15, NAVY); y -= 19;
  ctr('INSURANCE AGENCY', y, boldObl, 17, RED); y -= 15;
  ctr(o.branch || 'Speedy Insurance Agency', y, helv, 8, GRAY); y -= 11;
  ctr('(951) 472-0927  \u00b7  speedyins.com', y, helv, 8, GRAY); y -= 16;
  dash(y); y -= 20;
  ctr(isVoid ? t.voidTitle : t.title, y, bold, 11, isVoid ? AMBER : NAVY); y -= 26;
  ctr((isVoid ? '' : '- ') + slipMoney(o.amount), y, bold, 26, isVoid ? AMBER : BLACK); y -= 15;
  const headT = isCard ? t.headCard[isVoid ? 'void' : isPart ? 'partial' : 'full'] : t.headCash[isPart ? 'partial' : 'full'];
  ctr(slipFill(headT, { card: o.brand || 'card', last4: o.last4 || '????' }), y, bold, 9, isVoid ? AMBER : GREEN); y -= 22;
  const row = (label, value, isBold) => {
    draw(label, 29, y, helv, 8.5, GRAY);
    const f = isBold ? bold : helv;
    const avail = W - 29 - 29 - wid(label, helv, 8.5) - 10;
    let v = String(value == null ? '' : value);
    if (wid(v, f, 8.5) > avail) { while (v.length > 1 && wid(v + '\u2026', f, 8.5) > avail) v = v.slice(0, -1); v += '\u2026'; }
    draw(v, W - 29 - wid(v, f, 8.5), y, f, 8.5, BLACK); y -= 12;
  };
  row(t.date, o.stamp + ' PT');
  row(t.client, o.clientName || ('Client #' + o.clientNo), true);
  row(t.clientNo, String(o.clientNo));
  row(t.reason, t.reasons[o.reason] || t.reasons.other);
  if (!isVoid && o.refundRef) row(t.refundRef, o.refundRef, true);
  y -= 4; dash(y); y -= 14;
  if (isPart) { row(t.returned, slipMoney(o.amount), true); row(t.stands, slipMoney(o.stands), true); y -= 4; dash(y); y -= 14; }
  draw(t.record, 29, y, bold, 8.5, NAVY); y -= 13;
  row(t.ofPayment, slipMoney(o.of), true);
  row(t.paidOn, o.paidOn ? o.paidOn + ' PT' : '');
  if (o.payRef) row(t.payRef, o.payRef);
  row(t.by, o.by || 'Speedy Insurance');
  y -= 4; solid(y); y -= 16;
  const para = (str, f, sz, c) => {
    const words = String(str).split(' '); let line = '';
    for (const w of words) { const tt = line ? line + ' ' + w : w; if (wid(tt, f, sz) > W - 58) { draw(line, 29, y, f, sz, c); y -= sz + 3.5; line = w; } else line = tt; }
    if (line) { draw(line, 29, y, f, sz, c); y -= sz + 3.5; }
    y -= 5;
  };
  const vars = { ret: slipMoney(o.amount), of: slipMoney(o.of), stands: slipMoney(o.stands) };
  para(slipFill(isVoid ? t.voidLine : isPart ? t.partLine : t.fullLine, vars), bold, 8.5, BLACK);
  if (!isVoid) para(isCard ? t.bankCard : t.bankCash, helv, 8, GRAY);
  para(slipFill(t.quote, { ref: (isVoid || !o.refundRef) ? t.refPayment : t.refRefund }), helv, 8, GRAY);
  y -= 2; solid(y); y -= 16;
  ctr(`Speedy ref ${String(o.ledgerRef || '').slice(0, 8)}  \u00b7  ${o.late ? t.late : t.made}`, y, obl, 7, LIGHT); y -= 14;
  ctr(t.thanks, y, bold, 8, NAVY);
  return Buffer.from(await doc.save());
}

/* THE FACTS the slip is drawn from, read off the refund row and its parent. `voided`
   is stored on the row from Sep 15; an older row is judged the way the email was —
   a card refund inside 25 minutes of the charge is a void at Clover. */
function slipFactsOf(rf, parent, client) {
  const ex = rf.extra || {}, pex = (parent && parent.extra) || {};
  const isCard = rf.ref !== 'Cash returned' && (!!rf.txn_id || rf.ref === 'Clover refund');
  const ageMin = parent ? (new Date(rf.ts).getTime() - new Date(parent.ts).getTime()) / 60000 : 999;
  const voided = ex.voided != null ? ex.voided === true : (isCard && ageMin >= 0 && ageMin < 25);
  const amount = Math.abs(Number(rf.amount || 0));
  const part = ex.partial || {};
  const of = part.of != null ? Number(part.of) : (parent ? Number(parent.amount || 0) : amount);
  const partial = part.is_partial === true || (amount + 0.004 < of);
  const stands = part.refundable_after != null ? Number(part.refundable_after) : +(of - amount).toFixed(2);
  const name = client ? (client.business_name || [client.first_name, client.last_name].filter(Boolean).join(' ')) : '';
  const byName = AGENT_NAME[String(rf.agent || '').toLowerCase()] || String(rf.agent || '').split('@')[0] || '';
  return {
    kind: voided ? 'void' : partial ? 'partial' : 'full', method: isCard ? 'card' : 'cash',
    amount, of, stands, brand: String(pex.brand || 'CARD').toUpperCase(), last4: pex.last4 ? String(pex.last4) : '',
    clientName: name, clientNo: rf.client_id, reason: rf.refund_reason || 'other',
    refundRef: isCard ? (rf.txn_id || '') : '', stamp: slipStamp(rf.ts),
    paidOn: parent ? slipStamp(parent.ts) : '', payRef: parent ? (parent.ref || parent.txn_id || '') : '',
    by: byName, ledgerRef: rf.id, branch: pex.office ? `Speedy Insurance - ${pex.office}` : null,
    voided, partial,
  };
}

/* File the slip: HawkSoft attachment, vault row, event. Idempotent per refund and
   language — asking twice returns the row already there, with its bytes, so the email
   and the chip never make a second copy. Fail-soft on HawkSoft (the row records
   filed_hawksoft false and the chip says so); a vault failure is returned as null. */
async function fileRefundSlip(s, rf, lang, who, opts = {}) {
  lang = lang === 'es' ? 'es' : 'en';
  const have = await sbGet(s, `attachments?payment_id=eq.${encodeURIComponent(rf.id)}&doc_type=eq.refund_confirmation_${lang}&select=id,filename,file_b64,filed_hawksoft,created_at&limit=1`);
  const ex = (have.rows || [])[0];
  if (ex) return { id: ex.id, filename: ex.filename, filed_hawksoft: ex.filed_hawksoft === true, created_at: ex.created_at, existing: true,
    pdfBuf: ex.file_b64 ? Buffer.from(ex.file_b64, 'base64') : null };
  let parent = opts.parent || null;
  if (!parent && rf.refund_of) { const pr = await sbGet(s, `bridge_ledger?id=eq.${encodeURIComponent(rf.refund_of)}&select=id,ts,amount,ref,txn_id,extra`); parent = (pr.rows || [])[0] || null; }
  let client = opts.client || null;
  if (!client) { const cr = await sbGet(s, `clients?client_no=eq.${encodeURIComponent(rf.client_id)}&select=first_name,last_name,business_name`); client = (cr.rows || [])[0] || null; }
  const facts = slipFactsOf(rf, parent, client);
  const late = (Date.now() - new Date(rf.ts).getTime()) > 60 * 60 * 1000;
  const pdfBuf = await buildRefundSlipPdf({ ...facts, lang, late });
  ...
  const fname = `Refund_confirmation_${rf.client_id}_${String(rf.ts || '').slice(0, 10)}_${facts.amount.toFixed(2).replace('.', '-')}usd_${lang}.pdf`;
  const label = lang === 'es'
    ? (facts.kind === 'void' ? 'Confirmación de cancelación (Español)' : 'Confirmación de reembolso (Español)')
    : (facts.kind === 'void' ? 'Cancellation confirmation (English)' : 'Refund confirmation (English)');
  /* HawkSoft — the same headers the receipt uses; the note is what CMS shows beside it. */
  let filed = false; const refId = randomUUID();
  try {
    const { gzipSync } = await import('node:zlib');
    const b64h = str => Buffer.from(String(str), 'utf8').toString('base64');
    const note = `${facts.kind === 'void' ? 'CANCELLATION' : 'REFUND'} CONFIRMATION (${lang === 'es' ? 'Spanish' : 'English'}) — $${facts.amount.toFixed(2)}`
      + (facts.partial ? ` of the $${facts.of.toFixed(2)} payment` : '') + (facts.refundRef ? ` · Clover refund ${facts.refundRef}` : '')
      + ` · ${facts.stamp} PT · the slip the client can be handed. Generated by the Speedy platform from the refund record${late ? ' (after the fact)' : ''}; requested by ${who || 'the platform'}.`;
    const hr = await hsCall(`/vendor/agency/${AGENCY_ID}/client/${rf.client_id}/attachment?version=4.0`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/octet-stream', RefId: refId, TS: new Date().toISOString(),
        Desc: b64h(label.slice(0, 41)), LogNote: b64h(note), FileName: b64h(fname.replace(/\.pdf$/, '')), FileExt: 'pdf', Channel: '32' },
      body: gzipSync(pdfBuf) });
    filed = hr.status === 200 || hr.status === 202;
  } catch { filed = false; }
  const id = randomUUID();
  const ins = await sbInsert(s, 'attachments', [{
    id, client_no: rf.client_id, payment_id: rf.id, kind: 'refund_confirmation', doc_type: `refund_confirmation_${lang}`, doc_label: label,
    filename: fname, file_b64: pdfBuf.toString('base64'), sha256: sha, mime: 'application/pdf', bytes: pdfBuf.length,
    amount: facts.amount, uploaded_by: who ? String(who).slice(0, 120) : 'platform', filed_hawksoft: filed, hawksoft_refid: filed ? refId : null,
  }]);
  if (!ins.ok) return null;
  await fetch(`${s.base}/rest/v1/events`, { method: 'POST', headers: { ...s.hdrs, Prefer: 'return=minimal' },
    body: JSON.stringify({ ts: new Date().toISOString(), actor: who || 'platform', kind: 'refund.confirmation_filed', client_no: rf.client_id, source: opts.source || 'portal',
      payload: { refund_id: rf.id, payment_id: rf.refund_of || null, lang, attachment_id: id, filed_hawksoft: filed, late, kind: facts.kind, amount: facts.amount } }) });
  return { id, filename: fname, filed_hawksoft: filed, created_at: new Date().toISOString(), existing: false, late, pdfBuf };
}
  ...
  const res = { status: c => ({ json: b => ({ status: c, body: b }) }) };
      ...
      const refundRow = {
        ...
        extra: { partial: { is_partial: partial, of: collected, refundable_before: refundable, refundable_after: refundableAfter, carrier_share: carrierShare },
          /* a card refund inside 25 minutes is a void at Clover: nothing posts. Stored, so
             the slip, the email and the card all call it the same thing forever. */
          voided: isCard && ((Date.now() - new Date(row.ts).getTime()) / 60000) < 25,
      ...
      /* ---- THE SLIP, before the email so it rides on it. Fail-soft: a refund is
         never refused because a PDF did not build. ---- */
      let slip = null;
      try { slip = await fileRefundSlip(s, refundRow, 'en', me2, { parent: row }); } catch { slip = null; }
      ...
      if (nzChannel === 'email') {
        ...
        const r = await sendRefundEmail({
          ...
          attachments: slip && slip.pdfBuf ? [{ filename: slip.filename, content: slip.pdfBuf, contentType: 'application/pdf' }] : [] });
      ...
      await fetch(`${s.base}/rest/v1/events`, { method: 'POST', headers: { ...s.hdrs, Prefer: 'return=minimal' },
        body: JSON.stringify({ ts: stamp, actor: me2, kind: 'payment.refunded',
          ...
          payload: {
            ...
            confirmation: slip ? { id: slip.id, filed_hawksoft: slip.filed_hawksoft } : null,
          } }) });

      return res.status(200).json({ ok: true,
        refund_id: refundId,
        client_notice: notice,
        confirmation: slip ? { id: slip.id, filed_hawksoft: slip.filed_hawksoft, filename: slip.filename } : null,
        amount, method: isCard ? 'card' : 'cash',
...
if (view === 'portal_share_due') {
  ...
  if (req.method === 'POST') {
    ...
    /* THE SLIP ON DEMAND (Sep 15): the chip on the card and the Console button. Returns
    ...
    if (action === 'refund_slip') {
      ...
      let slip = null;
      try { slip = await fileRefundSlip(s, rf, lang, me2, { source: b3.source === 'console' ? 'console' : 'portal' }); } catch (e) { slip = null; }
      if (!slip) return res.status(500).json({ ok: false, error: 'The confirmation could not be made. Nothing was filed.' });
      return res.status(200).json({ ok: true, id: slip.id, lang, filename: slip.filename, filed_hawksoft: slip.filed_hawksoft, created_at: slip.created_at, existing: slip.existing === true, late: slip.late === true });
    }
    ...
    if (action === 'notify_refund') {
      ...
      const rf = (rr.rows || [])[0];
      if (!rf) return res.status(404).json({ ok: false, error: 'No such refund.' });
      const pr = rf.refund_of ? await sbGet(s, `bridge_ledger?id=eq.${encodeURIComponent(rf.refund_of)}&select=id,ts,amount,ref,kind,txn_id,extra`) : { rows: [] };
      ...
      let slip = null;
      try { slip = await fileRefundSlip(s, rf, 'en', me2, { parent, source: 'console' }); } catch { slip = null; }
      const r = await sendRefundEmail({
        to, name: cn.business_name || cn.first_name || '',
        amount, method: isCard ? 'card' : 'cash', voided: (rf.extra || {}).voided === true,
        attachments: slip && slip.pdfBuf ? [{ filename: slip.filename, content: slip.pdfBuf, contentType: 'application/pdf' }] : [],
